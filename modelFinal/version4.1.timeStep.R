...
pb <- progress_bar$new(
  ...
  total = length(variationstimestep), clear = FALSE)
opts <- list(progress=progress)
bind3d <- function(...) abind(..., along = 3)

version <- paste0("version4.1.timestep")

start <- Sys.time()
out <-foreach(timestep=iter(variationstimestep), .options.snow=opts, .packages = c("deSolve")) %dopar% {
  set.seed(5)
...
for (i in 1:length(out)) {
  ...
  tmp$timestep <- as.factor(c(variationstimestep[i]))
...
## -plots---------------------------------------------------------------------------------------------------
{
  h.plot <- ggplot(outData, aes(simHours, h, group=timestep)) +
    geom_line(size = 0.5, aes(colour = timestep)) +
  ...
  n_Bottles.plot <- ggplot(outData, aes(x = simHours, y = n_Bottles, group = timestep)) +
    geom_line(size = 0.5, aes(colour = timestep)) +
  ...
  Q_out.plot <- ggplot(outData, aes(x = simHours, y = Q_out, group = timestep)) +
    geom_line(size = 0.5, aes(colour = timestep)) +
    ylab("Outflow (Q_out)") + 
    scale_x_continuous(name = "simulation time in hours", 
                       breaks = c(6, 9, 12, 15, 18), labels = function(x) {paste0(x, "h")}) +
  ...
  satisfactionRate.plot <- ggplot(outData, aes(x = simHours, y = satisfactionRate, group = timestep)) +
    geom_line(size = 0.5, aes(colour = timestep)) +
  ...
  waitingLineLength.plot <- ggplot(outData, aes(x = simHours, y = waitingLineLength, group = timestep)) +
    geom_line(size = 0.5, aes(colour = timestep)) +
  ...
  n_taps.plot <- ggplot(outData, aes(x = simHours, y = n_taps, group = timestep)) +
    geom_line(size = 0.5, aes(colour = timestep)) +
  ...
  gglegend <- get_legend(
    ggplot(outData, aes(simHours, c, group=timestep)) +
      geom_line(size = 0.5, aes(colour = timestep)) +
...
par(mfrow = c(1,1))
for (timestep in variationstimestep) {
  png(filename = paste0("./plots/vartimestep/", version, "-", timestep, ".png"), 
      width = 1500, height = 1200, res = 150)
  # filter outData for specific timestep
  out.tmp <- outData[which(outData$timestep == timestep), ]
