...
variationsn_taps <- c(5, 8, 10, 12, 15, 20, 25)
...
version <- paste0("version4.1.n_taps.", socialNorm, ".", timestep, "secSteps")
...
## -plots---------------------------------------------------------------------------------------------------
{
  h.plot <- ggplot(outData, aes(simHours, h, group=n_taps)) +
  ...
  n_Bottles.plot <- ggplot(outData, aes(x = simHours, y = n_Bottles, group = n_taps)) +
    geom_line(size = 0.5, aes(colour = n_taps)) +
  ...
  Q_out.plot <- ggplot(outData, aes(x = simHours, y = Q_out, group = n_taps)) +
    geom_line(size = 0.5, aes(colour = n_taps)) +
    ylab("Outflow (Q_out)") + 
    scale_x_continuous(name = "simulation time in hours", 
                       breaks = c(6, 9, 12, 15, 18), labels = function(x) {paste0(x, "h")}) +
  ...
  n_taps.plot <- ggplot(outData, aes(x = simHours, y = n_taps, group = n_taps)) +
    geom_line(size = 0.5, aes(colour = n_taps)) +
...
for (n_taps in variationsn_taps) {
  png(filename = paste0("./plots/varn_taps/", version, "-", n_taps, ".png"), 
