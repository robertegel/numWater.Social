...
pb <- progress_bar$new(
  ...
  total = length(variationsA_roof), clear = FALSE)
opts <- list(progress=progress)
bind3d <- function(...) abind(..., along = 3)

version <- paste0("version4.1.A_roof.", socialNorm, ".", timestep, "secSteps")

start <- Sys.time()
out <-foreach(A_roof=iter(variationsA_roof), .options.snow=opts, .packages = c("deSolve")) %dopar% {
  set.seed(5)
  ...
  startParameters <- defineStartParameters(A_roof = A_roof)
...
for (i in 1:length(out)) {
  ...
  tmp$A_roof <- as.factor(c(variationsA_roof[i]))
...
## -plots---------------------------------------------------------------------------------------------------
{
  h.plot <- ggplot(outData, aes(simHours, h, group=A_roof)) +
  ...
  n_Bottles.plot <- ggplot(outData, aes(x = simHours, y = n_Bottles, group = A_roof)) +
    geom_line(size = 0.5, aes(colour = A_roof)) +
  ...
  Q_out.plot <- ggplot(outData, aes(x = simHours, y = Q_out, group = A_roof)) +
    geom_line(size = 0.5, aes(colour = A_roof)) +
    ylab("Outflow (Q_out)") + 
    scale_x_continuous(name = "simulation time in hours", 
                       breaks = c(6, 9, 12, 15, 18), labels = function(x) {paste0(x, "h")}) +
  ...
  satisfactionRate.plot <- ggplot(outData, aes(x = simHours, y = satisfactionRate, group = A_roof)) +
    geom_line(size = 0.5, aes(colour = A_roof)) +
  ...
  waitingLineLength.plot <- ggplot(outData, aes(x = simHours, y = waitingLineLength, group = A_roof)) +
    geom_line(size = 0.5, aes(colour = A_roof)) +
  ...
  n_taps.plot <- ggplot(outData, aes(x = simHours, y = n_taps, group = A_roof)) +
    geom_line(size = 0.5, aes(colour = A_roof)) +
  ...
  gglegend <- get_legend(
    ggplot(outData, aes(simHours, c, group=A_roof)) +
      geom_line(size = 0.5, aes(colour = A_roof)) +
...
for (A_roof in variationsA_roof) {
  png(filename = paste0("./plots/varA_roof/", version, "-", A_roof, ".png"), 
      width = 1500, height = 1200, res = 150)
  # filter outData for specific A_roof
  out.tmp <- outData[which(outData$A_roof == A_roof), ]
