...
version <- paste0("version4.1.z.", socialNorm, ".", timestep, "secSteps")
...
## -plots---------------------------------------------------------------------------------------------------
{
  h.plot <- ggplot(outData, aes(simHours, h, group=z)) +
  ...
  n_Bottles.plot <- ggplot(outData, aes(x = simHours, y = n_Bottles, group = z)) +
    geom_line(size = 0.5, aes(colour = z)) +
  ...
  Q_out.plot <- ggplot(outData, aes(x = simHours, y = Q_out, group = z)) +
    geom_line(size = 0.5, aes(colour = z)) +
    ylab("Outflow (Q_out)") + 
    scale_x_continuous(name = "simulation time in hours", 
                       breaks = c(6, 9, 12, 15, 18), labels = function(x) {paste0(x, "h")}) +
  ...
  n_taps.plot <- ggplot(outData, aes(x = simHours, y = n_taps, group = z)) +
    geom_line(size = 0.5, aes(colour = z)) +
